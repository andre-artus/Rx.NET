...
$signClientSettings = Join-Path (Join-Path $scriptPath.Parent.Parent.FullName "scripts") "SignClientSettings.json"
...
if($hasSignClientSecret) {
  ...
  foreach ($nupkg in $nupgks) {
    ...
    dotnet $appPath 'zip' -c $signClientAppPath -i $nupkg -s $env:SignClientSecret -n 'Rx.NET' -d 'Reactive Extensions for .NET' -u 'http://reactivex.io/' 
